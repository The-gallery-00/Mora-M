...
const TYPE_LABEL: Record<ScheduleDocType, string> = {
  TICKET: "티켓",
  POSTER: "포스터",
};
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={[
        ...
        .join(", ")}
      onPress={onPress}
      className={`flex-row items-center gap-2 px-3 py-4 ${ROW_CLASS[docType]}`}
      style={({ pressed }) => [
        { minHeight: height },
        style,
    ...
    >
      ...
      <View
        className="items-center rounded-xs bg-bg-elevated py-0.5"
        style={{ width: 44 }}
      >
        <Text
