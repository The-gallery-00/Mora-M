...
| App activity | submitted search query, searched document type and search timestamp | PostgreSQL `search_histories` | recent search history and search UX | user can clear all search history; account deletion cascades; no automatic TTL |
| App activity | deadline/schedule notification title, message, source document reference, read status and notification preferences | PostgreSQL `notifications`, `notification_settings` | dashboard reminders and notification settings | notification deletion and account deletion remove rows; no automatic TTL |
