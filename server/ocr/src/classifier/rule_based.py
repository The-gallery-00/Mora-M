...
# 중앙부처는 "…부" 로 끝나지만 '부' 를 접미사로 받으면 오탐이 폭발한다
# (일반부/학생부/상단부…). 이름이 닫힌 집합이므로 통째로 나열한다.
_MINISTRY = (
    r"기획재정부|교육부|과학기술정보통신부|외교부|통일부|법무부|국방부|행정안전부|"
    r"국가보훈부|문화체육관광부|농림축산식품부|산업통상자원부|보건복지부|환경부|"
    r"고용노동부|여성가족부|국토교통부|해양수산부|중소벤처기업부|기후에너지환경부"
)
_ORG_SUFFIX = re.compile(
    ...
    # 지자체·군부대·학교. 실측에서 ML 만 맞추고 규칙이 통째로 놓치던 형태다
    # (용인특례시 / 해군 제1함대사령부 / 시흥시청소년수련관).
    r"특례시|광역시|자치시|자치도|교육지원청|사령부|"
    r"고등학교|중학교|초등학교|수련관)"
    r"\s*(?:the\s*\S+)?\s*[.,)\]]?\s*$"
...
# 라벨 접미에 "기관/단체" 를 넣는다 — 포스터 하단 표는 '주최-주관기관' 처럼
# 라벨 자체를 헤더로 적는 일이 흔하고, 그것이 값으로 새어 나갔다.
_ORG_LABEL_SUFFIX = r"(?:자|측|처|사|기관|단체)?"
# 라벨 결합은 슬래시만이 아니라 하이픈·가운뎃점도 쓴다('주최-주관', '주최·주관').
_ORG_LABEL_CORE = (
    r"(?:주최|주관|후원|협찬)(?:\s*[/／·・\-–]\s*(?:주최|주관|후원|협찬))*"
    + _ORG_LABEL_SUFFIX
...
# 기관명이 아니라 **문장**임을 알리는 종결어미.
#
# 조사(이/가/은/는)와 '서'·'고'는 일부러 뺐다 — '세무서'·'경찰서'·'소방서',
# '○○공고'·'○○여고' 처럼 그 글자로 끝나는 기관명이 실재하고, 그것까지 죽이면
# 오답을 줄이려다 정답을 잃는다. 여기 남긴 것은 기관명에 나타나지 않는 어미뿐이다.
_ORG_SENTENCE_TAIL = re.compile(
    r"(?:며|음|임|니다|한다|된다|있다|없다|합니다|됩니다|하다|되다)\s*[.!?]?\s*$"
)
...
def _org_finalize(v: str) -> str:
    ...
    if len(v.split()) > 6:              # 과도 길이 → 앞 6토큰
        v = " ".join(v.split()[:6])
    # **라벨과 문장은 주최자가 아니다.** _clean_organizer 의 반환 경로가 넷이라
    # 각자 걸러서는 새는 곳이 생겼다(실측: '주최-주관기관' 이 표 헤더인데 값이 됐고,
    # '저작권은 주최측이가지며' 는 문장인데 값이 됐다). 모든 경로가 이 함수를
    # 거치므로 최종 판정을 여기 한 곳에 둔다. 빈 값이면 services._aggregate 가
    # 필드를 빼며, 그것이 라벨 조각을 남기는 것보다 사람이 고쳐 쓰기 쉽다.
    if _ORG_LABEL_ONLY.match(v) or _ORG_SENTENCE_TAIL.search(v):
        return ""
    return v
...
def _clean_organizer(text: str) -> str:
    ...
    # 최후 폴백으로 원문을 그대로 돌려주던 자리다. "데이터 소실 방지" 의도였지만
    # 실제로는 라벨을 품은 **문장**이 주최자 값이 됐다 — '주최측이가지며'(저작권
    # 문구)가 대표적이다. 조사·어미로 끝나는 말은 기관명이 아니다. 빈 값이면
    # services._aggregate 가 필드를 아예 빼므로, 사람이 고쳐 쓰기 더 쉽다.
    if _ORG_SENTENCE_TAIL.search(raw):
        return ""
