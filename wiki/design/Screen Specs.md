...
| 진입 | SCR-25 설정 > 앱 정보의 **버전 라벨 5회 탭**. 부트 실패 시 SCR-01 에러 카드의 `서버 주소 확인` 버튼 |
...
**노출 규칙 (결정)** — `__DEV__ === false` **이고** `EXPO_PUBLIC_ALLOW_SERVER_OVERRIDE !== '1'` 이면 라우트 자체를 등록하지 않는다. 릴리스 APK에서 5회 탭해도 아무 일도 일어나지 않는다.
...
│                                    │
│ ┌──────────────────────────────┐   │
│ │        연결 테스트           │   │  Button primary
│ └──────────────────────────────┘   │
│                                    │
│ ┌────────────────────────────────┐ │
│ │ ✓ OCR :8000   MORA OCR v3.0    │ │  결과 카드
│ │ ✓ API :8080   401 (정상 응답)   │ │
│ │ ✗ API :8080   타임아웃 (3s)     │ │  실패 시 text-danger
│ └────────────────────────────────┘ │
│                                    │
│ 기본값으로 되돌리기                 │  ghost / textMuted
│▓▓ safe-bottom ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓ │
└────────────────────────────────────┘
```
...
| 폼 | API base URL 입력 | CMP-03 | `keyboardType="url"`, `autoCapitalize="none"`, `http://` 스킴 필수 |
| 폼 | OCR base URL 입력 | CMP-03 | 동일 |
| 액션 | `연결 테스트` | CMP-01 | primary. 두 프로브를 병렬 실행 |
| 결과 | 프로브 결과 2행 | CMP-15 / CMP-27 | 성공 `✓ + 응답 요약`, 실패 `✗ + 사유` |
| 하단 | `기본값으로 되돌리기` | CMP-01(ghost) | `.env` 값 복원 후 즉시 적용 |
...
| 트리거 | 동작 | 피드백 | 실패 시 |
|---|---|---|---|
| URL 입력 후 blur | 형식 검증 → AsyncStorage(`mora.dev.apiBase` / `mora.dev.ocrBase`) 저장 → **앱 재시작 없이 즉시 적용** | selection 햅틱 | 형식 오류 문구 |
| `연결 테스트` 탭 | `GET {OCR}/`(API-65, 3s) + `GET {API}/auth/me`(API-03, 3s) 병렬 | 버튼 스피너 | 행별 `✗` + 사유 |
| `기본값으로 되돌리기` 탭 | 오버라이드 키 2개 삭제 → `.env` 값 복원 | Success 햅틱 + 토스트 `기본 서버 주소로 되돌렸습니다.` | — |
| 백 | pop → SCR-25 | — | — |
...
| API | 시점 | 타임아웃 | 캐시 |
|---|---|---|---|
| API-65 `GET {OCR}/` | `연결 테스트` | 3s | 캐시 안 함 |
| API-03 `GET {API}/auth/me` | `연결 테스트` | 3s | 캐시 안 함. **401도 성공 판정** (서버가 살아 있다는 뜻) |
| (로컬) AsyncStorage `mora.dev.apiBase` / `mora.dev.ocrBase` | 입력 blur | — | 영구. 토큰이 아니므로 SecureStore 아님 |
